...
:main
echo ============================================
echo  Teste Rifa - Fluxo Completo
echo ============================================
...
echo === 4b. Participar (numero 7) ===
curl -s -X POST "%BASE_URL%/api/participacoes" -H "Authorization: Bearer %TOKEN_JOG%" -H "Content-Type: application/json" -d "{\"jogoId\":\"%JOGO_ID%\",\"dadosParticipacao\":{\"numeros\":[7]},\"quantidade\":1,\"metodoPagamento\":\"saldo\",\"dadosCliente\":{\"nome\":\"Jogador\",\"telefone\":\"912345678\"}}" > _resp_part.json
...
echo === 6. Sortear (POST) ===
curl -s -X POST "%BASE_URL%/api/sorteios" -H "Authorization: Bearer %TOKEN%" -H "Content-Type: application/json" -d "{\"jogoId\":\"%JOGO_ID%\",\"clientSeed\":\"teste456\"}" > _resp_sortear.json
