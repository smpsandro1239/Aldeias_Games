...
export default function TombolaPage() {
  ...
  useEffect(() => {
    if (jogo && config.numeroInicial && config.numeroFinal) {
      // Compute all numbers in the range
      const allNumbers = [];
      for (let i = config.numeroInicial; i <= config.numeroFinal; i++) {
        allNumbers.push(i);
      }
      // Remove occupied numbers
      const available = allNumbers.filter(num => !numerosOcupados.includes(num));
  ...
  const fetchJogo = async () => {
    try {
      ...
      let url = "/api/jogos?ativos=true&tipo=tombola";
      if (jogoId) {
      ...
      if (jogoData) {
        ...
        let configData: Record<string, any> = { numeroInicial: 1, numeroFinal: 50, numeroBlocos: 1, permitirStripe: false, valorPremios: null, dataSorteio: null, horaSorteio: null, localSorteio: null };
        if (jogoData.configuracao) {
          if (typeof jogoData.configuracao === 'string') {
            try {
              configData = JSON.parse(jogoData.configuracao);
        if (jogoData.dataSorteio !== undefined) configData.dataSorteio = jogoData.dataSorteio;
        if (jogoData.horaSorteio !== undefined) configData.horaSorteio = jogoData.horaSorteio;
        if (jogoData.localSorteio !== undefined) configData.localSorteio = jogoData.localSorteio;
            } catch (e) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (participante.notificacao === "whatsapp" && participante.telefone) {
          ...
          const body = encodeURIComponent(`🎉 Cartela Confirmada!\n\nRifa: ${jogo.nome}\nNúmeros: ${numerosSelecionados.join(", ")}\n${config.dataSorteio ? `Sorteio: ${config.dataSorteio}${config.horaSorteio ? ` às ${config.horaSorteio}` : ''}` : ''}\n${config.localSorteio ? `Local: ${config.localSorteio}` : ''}\n\nCódigo de Verificação: ${hash || 'Consulte seu perfil'}\n\nObrigado por apoiar!`);
  ...
  if (participacaoConfirmada) {
    ...
    const numerosVendidas = (jogo?.stockInicial || 0) - (jogo?.stockAtual || 0);
       ...
       <LayoutHeader>
         <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="font-serif text-2xl text-accent font-bold">Cartela Confirmada!</h2>
            <p className="text-muted-foreground mt-2">Obrigado pelo seu jogo</p>
          </div>
          ...
          <div className="bg-surface-container rounded-3xl overflow-hidden mb-6">
            <div className="p-6 md:p-8 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-surface-container-high rounded-xl p-4 text-center">
                  <p className="text-[10px] text-on-surface/50 uppercase">Cartelas Jogadas</p>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-surface-container-high rounded-xl p-3 text-center">
                  <p className="text-[10px] text-on-surface/50 uppercase">Total de Cartelas</p>
                  <p className="text-lg font-bold">{jogo?.stockInicial || 0}</p>
                </div>
                <div className="bg-surface-container-high rounded-xl p-3 text-center">
                  <p className="text-[10px] text-on-surface/50 uppercase">Vendidas</p>
                  <p className="text-lg font-bold text-primary">{numerosVendidas}</p>
                </div>
                <div className="bg-surface-container-high rounded-xl p-3 text-center">
       ...
       <div className="min-h-screen bg-background text-foreground">
         <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-xl border-b border-primary/10 flex items-center justify-between px-4 py-3">
           <div className="flex items-center gap-3">
             ...
             <h1 className="font-serif text-xl tracking-wide text-accent font-bold italic">A Tua Tombola</h1>
           </div>
         </header>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <h3 className="text-2xl font-headline font-bold text-on-surface">
                {jogo?.premio?.nome || "Vale de 300€ + Cabaz de Produtos Locais"}
              </h3>
            </div>
          ...
          <div className="bg-surface-container rounded-2xl p-6 border border-primary/20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-surface-container-high rounded-xl p-4">
                <p className="text-[10px] text-on-surface/50 uppercase mb-1">Total de Cartelas</p>
                <p className="text-xl font-bold">{jogo?.stockInicial || 0}</p>
              </div>
              <div className="bg-surface-container-high rounded-xl p-4">
                <p className="text-[10px] text-on-surface/50 uppercase mb-1">Números Vendidas</p>
              ...
              {numerosDisponiveis.map((num) => {
                ...
                return (
                  <button
                    ...
                    `}
                    title={isJogado ? "Este número já foi marcado por si" : isOcupado ? "Número já marcado" : `Número ${num}`}
                  >
                    {num}
                  </button>
                );
