  ...
  const criarAposta = async (pago: boolean, metodoPagamento: string = "dinheiro") => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setApostaConfirmada({
          id: novaAposta.data.id,
          numeros: pagamentoPendente.numeros,
          labels,
          pago,
          nome: jogadorForm.nome,
        });
  ...
  if (loading) {
   ...
   return (
     <LayoutHeader>
      ...
      {/* Dialog de Confirmação da Aposta */}
      <Dialog open={!!apostaConfirmada} onOpenChange={() => setApostaConfirmada(null)}>
        <DialogContent className="sm:max-w-md bg-surface-container border-primary/20">
          ...
          {apostaConfirmada && (
            <div className="space-y-4 py-2">
              ...
              <Button
                onClick={() => setApostaConfirmada(null)}
                className="w-full"
              >
                Fechar
              </Button>
