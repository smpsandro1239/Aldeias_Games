...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!token || !novaPassword) {
      return NextResponse.json({ error: 'Token e nova password são obrigatórios' }, { status: 400 });
    }

    const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]).{12,}$/;
    if (!passwordRegex.test(novaPassword)) {
      return NextResponse.json({ error: 'Password deve ter pelo menos 12 caracteres e conter: 1 maiúscula, 1 minúscula, 1 número e 1 carácter especial' }, { status: 400 });
    }
    ...
    if (!user) return NextResponse.json({ error: 'Utilizador não encontrado' }, { status: 400 });
