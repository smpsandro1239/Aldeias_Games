...
// Constants for modes to avoid magic strings
const MODO_JOGO = {
  INDIVIDUAL: 'individual',
  CARTAO: 'cartao'
} as const;

type ModoJogo = typeof MODO_JOGO[keyof typeof MODO_JOGO];
...
export function PoioDaVacaModal({
  ...
  const [modo, setModo] = useState<ModoJogo>(MODO_JOGO.INDIVIDUAL);
  ...
  }, [selecao, modo, numerosPorLetra, isOcupado, isJogadoPorMim, isSelecionado]);
  ...
  }, [isOcupado, isJogadoPorMim, isSelecionado]);
  ...
  }, [selecao, modo, letras, numerosPorLetra, precoCartao, precoIndividual]);
  ...
  const handleConfirm = useCallback(() => {
    ...
    setSelecao([]);
  }, [selecao, onSelect, onConfirmWithPayment, onConfirm]);

  const handleModoChange = useCallback((value: string) => {
    setModo(value as ModoJogo);
  }, []);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col p-0" aria-describedby="poio-modal-description">
        <div className="relative bg-gradient-to-br from-emerald-950 via-teal-900 to-cyan-950 p-6 pb-4">
          <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNDAiIGhlaWdodD0iNDAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMjAiIGN5PSIyMCIgcj0iMiIgZmlsbD0id2hpdGUiIG9wYWNpdHk9IjAuMDUiLz48L3N2Zz4=')] opacity-30" aria-hidden="true" />
                      ...
                      {Array.from({ length: numerosPorLetra }, (_, i) => i + 1).map((numero) => {
                        ...
                        return (
                          <motion.button
                            ...
                            disabled={disabled}
                            aria-label={getNumeroAriaLabel(letra, numero)}
                          ...
                          >
                            <span className="relative">
                              ...
                              {selecionado && (
                                ...
                                >
                                  <CheckCircle className="w-4 h-4 text-foreground" aria-hidden="true" />
                                </motion.span>
                              )}
                            </span>
                          </motion.button>
