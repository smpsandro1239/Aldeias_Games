...
export function PremioModal({
  ...
  const handleChange = useCallback((field: keyof Premio, value: string | number) => {
    setFormData(prev => ({ ...prev, [field]: value }));
  }, []);

  const handleValorChange = useCallback((value: string) => {
    setFormData(prev => ({ ...prev, valorDinheiroAlternative: safeParseFloat(value, 0) }));
  }, []);
  ...
  }, [formData, premio, token, aldeiaId, jogoId, onSave, onOpenChange]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-surface-container border-primary/20 p-0 max-w-md overflow-hidden" aria-describedby="premio-modal-description">
        <DialogHeader className="p-6 pb-0">
          <DialogTitle className="flex items-center gap-2">
            <Gift className="h-5 w-5 text-primary" aria-hidden="true" />
            {premio ? "Editar Prémio" : "Novo Prémio"}
          </DialogTitle>
          <DialogDescription id="premio-modal-description">
            {premio
        ...
        <div className="p-6 space-y-4">
          {/* Nome */}
          <div>
            <Label htmlFor="nome" className="text-xs text-muted-foreground uppercase tracking-wider mb-2 block">
              Nome do Prémio *
            </Label>
            <Input
              id="nome"
          ...
          {/* Descrição */}
          <div>
            <Label htmlFor="descricao" className="text-xs text-muted-foreground uppercase tracking-wider mb-2 block">
              Descrição
            </Label>
            <Input
              id="descricao"
          ...
          {/* Valor em Dinheiro */}
          <div>
            <Label htmlFor="valor" className="text-xs text-muted-foreground uppercase tracking-wider mb-2 block">
              Valor em Dinheiro (alternativa)
            </Label>
            <div className="relative">
              <Euro className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-primary" aria-hidden="true" />
              <Input
                id="valor"
                type="number"
                min="0"
            ...
            <p id="valor-description" className="text-xs text-muted-foreground mt-1">
          ...
          {/* Imagem URL */}
          <div>
            <Label htmlFor="imagemUrl" className="text-xs text-muted-foreground uppercase tracking-wider mb-2 block">
              URL da Imagem
            </Label>
            <div className="relative">
          ...
          <div className="bg-secondary/10 rounded-xl p-4 flex items-start gap-3">
            <Gift className="h-5 w-5 text-secondary mt-0.5 flex-shrink-0" aria-hidden="true" />
    ...
    <div className="space-y-2">
      {premios.length === 0 ? (
        <div className="bg-surface-container rounded-xl p-6 text-center border border-outline-variant/10">
          <Gift className="h-10 w-10 text-primary mx-auto mb-2 opacity-50" aria-hidden="true" />
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                aria-label={`Mover ${premio.nome} para cima`}
              >
              ...
              <button
                ...
                className="p-2 rounded-lg hover:bg-surface-container-low disabled:opacity-30 transition-colors"
                aria-label={`Mover ${premio.nome} para baixo`}
              >
              ...
              <button
                ...
                aria-label={`Eliminar ${premio.nome}`}
              >
